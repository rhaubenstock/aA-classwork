...
def harrison_ford
  ...
  Movie
    ...
    .where.not(castings: {ord: 1}) # .where("castings.ord != 1 AND actors.name = 'Harrison Ford'")
...
def biggest_cast
  # Consider the following:
  #
  # Actor # SELECT * FROM actors
  #   .joins(:movies) # SELECT * FROM actors JOIN movies 
  #   .group('actors.id') # SELECT * FROM actors JOIN ... GROUP BY actors.id
...
def directed_by_one_of(them)
  ...
  # N + 1 Version:
  # arr = []
  # them.each do |director_name|
  #   Movie
  #   .select(:id, :title)
  #   .joins(:director)
  #   .where(actors: {name: director_name})
  #   .each {|movie| arr << movie}
  # end
  # arr
