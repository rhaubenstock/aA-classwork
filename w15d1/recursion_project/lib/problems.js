...
function pow(base, exponent) {
    ...
    if (exponent % 2 === 0){
        const squareRoot = pow(base, exponent/2);
        return squareRoot * squareRoot;
    }
    const almostSquareRoot = pow(base, (exponent-1)/2);
    return base * almostSquareRoot * almostSquareRoot;
}
...
function flatten(data) {
    ...
    if (data[0] instanceof Array) return flatten(data[0]).concat(flatten(data.slice(1)));
    return [data[0]].concat(flatten(data.slice(1)));
}
...
function fileFinder(directories, targetFile) {
    ...
    if (directories[targetFile] !== undefined) return true;
    return Object.values(directories).some(directory => fileFinder(directory, targetFile));
}

console.log(fileFinder({"it":1}, "it"));
...
function pathFinder(directories, targetFile) {
    ...
    if (directories[targetFile] !== undefined) return targetFile;
    let directory;
    let path;
    for (directory in directories){
        path = pathFinder(directories[directory], targetFile);
        if (!(path === null)){
            return directory + (path[0] === "/" ? "" : "/") + path;
        }
    }
    return null;
}
